...
{
    public class 考生完形填空回答:考生考试回答
    {
        #region 属性

        public List<完形空格回答> 完形空格回答集合
        {
            get;
            set;
        }
        ...
        public static IQueryable<考生完形填空回答> 考生完形填空回答查询
        {
            get
            {
                LoveKaoExamEntities db = new LoveKaoExamEntities();
                return db.考生考试回答表.Select(a => new 考生完形填空回答
                    {
                        ID = a.ID,
                        该题的得分 = a.该题的得分,
                        该题的用时 = a.该题的用时,
        ...
        public override void 保存考生考试回答(LoveKaoExamEntities db)
        {
            ...
            for (int i = 0; i < this.完形空格回答集合.Count; i++)
            {
                考生多空选择题回答表 spaceAnswer = new 考生多空选择题回答表();
                spaceAnswer.ID = Guid.NewGuid();
                spaceAnswer.该空ID = this.完形空格回答集合[i].空格ID;
                spaceAnswer.该空的回答ID = this.完形空格回答集合[i].回答的选项ID;
                memberTestAnswer.考生多空选择题回答表.Add(spaceAnswer);
            }
